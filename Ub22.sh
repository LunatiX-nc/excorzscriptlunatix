...
username=$(curl https://raw.githubusercontent.com/LunatiX-nc/registr/main/ip | grep $Ip_Vps | awk '{print $2}')
echo "$username" >/usr/bin/user

# Save Expired Detail in github to /usr/bin/e
expx=$(curl https://raw.githubusercontent.com/LunatiX-nc/registr/main/ip | grep $Ip_Vps | awk '{print $3}')
echo "$expx" >/usr/bin/e
...
Exp1=$(curl https://raw.githubusercontent.com/LunatiX-nc/registr/main/ip | grep $Ip_Vps | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
if [[ $(cat /etc/os-release | grep -w ID | head -n1 | sed 's/=//g' | sed 's/"//g' | sed 's/ID//g') == "ubuntu" ]]; then
    ...
    # Menambahkan repository terbaru untuk HAProxy
    ...
    # Menambahkan kunci repository terbaru
    ...
    # Menambahkan repository terbaru
    ...
    echo "Unsupported operating system. Please check manually."
fi
}
clear
...
clear
restart_system() {
USRSC=$(wget -qO- https://raw.githubusercontent.com/LunatiX-nc/registr/main/ip | grep $ipsaya | awk '{print $2}')
EXPSC=$(wget -qO- https://raw.githubusercontent.com/LunatiX-nc/registr/main/ip | grep $ipsaya | awk '{print $3}')
...
cat >/etc/xray/update_geo.sh <<-END
#!/bin/bash

# Direktori penyimpanan file
XRAY_DIR="/usr/local/share/xray"

# URL sumber untuk GeoSite dan GeoIP
...
update_file() {
    ...
    echo "Memperbarui $name..."
    if wget -q -O "$destination" "$url"; then
        echo "$name berhasil diperbarui."
    else
        echo "Gagal memperbarui $name. Periksa koneksi internet Anda atau URL sumber."
    fi
}

# Perbarui GeoSite.dat
update_file "$GEOSITE_URL" "$XRAY_DIR/geosite.dat"

# Perbarui GeoIP.dat
update_file "$GEOIP_URL" "$XRAY_DIR/geoip.dat"
END

chmod +x /etc/xray/update_geo.sh
bash /etc/xray/update_geo.sh
clear

echo "0 3 * * * /etc/xray/update_geo.sh" > /etc/crontab
}
