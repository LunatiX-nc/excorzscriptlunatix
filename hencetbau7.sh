...
install_dependencies() {
    ...
    apt-get install -y curl wget git build-essential docker.io
}

# Fungsi untuk menjalankan container Docker
run_docker_container() {
    echo "Menjalankan container Docker..."
    if [[ "$OS" == "debian" && ("$VERSION" == "11" || "$VERSION" == "12") ]]; then
        docker run -it debian:$VERSION /bin/bash
    elif [[ "$OS" == "ubuntu" && ("$VERSION" == "22.04" || "$VERSION" == "24.04") ]]; then
        docker run -it ubuntu:$VERSION /bin/bash
    else
        echo "Container untuk $OS $VERSION tidak tersedia."
        exit 1
    fi
}

# Fungsi untuk penyesuaian berdasarkan versi OS
configure_os() {
    if [[ "$OS" == "debian" ]]; then
        case "$VERSION" in
            "11"|"12")
                echo "Konfigurasi untuk Debian $VERSION"
                ;;
            *)
        ...
        case "$VERSION" in
            "22.04"|"24.04")
                echo "Konfigurasi untuk Ubuntu $VERSION"
                ;;
            *)
...
# Fungsi utama untuk menjalankan installer
main() {
    detect_os
    update_system
    install_dependencies
